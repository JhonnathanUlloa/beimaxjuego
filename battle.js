...
// Screen flash effect — supports type-specific colors and effectiveness
function screenFlash(type) {
    ...
    flash.classList.remove('flash-active', 'flash-super', 'flash-crit',
        'flash-electric', 'flash-fire', 'flash-water', 'flash-steel', 'flash-psychic');
    void flash.offsetWidth;

    if (type === 'super' || type === 'crit') {
        flash.classList.add('flash-active', type === 'super' ? 'flash-super' : 'flash-crit');
    } else {
        // type-specific element flash
        flash.classList.add('flash-active', `flash-${type}`);
    }

    setTimeout(() => flash.classList.remove(
        'flash-active', 'flash-super', 'flash-crit',
        'flash-electric', 'flash-fire', 'flash-water', 'flash-steel', 'flash-psychic', 'flash-neutral'
    ), 400);
}

function spawnChargeEffect(who, moveType) {
    const robot = document.getElementById(who === 'player' ? 'battlePlayerRobot' : 'battleOpponentRobot');
    if (!robot) return;
    const cls = `robot-charging-${moveType || 'neutral'}`;
    robot.classList.add(cls);
    const dur = { electric: 300, fire: 350, water: 400, steel: 150, psychic: 500, neutral: 260 };
    setTimeout(() => robot.classList.remove(cls), dur[moveType] || 300);
}
...
async function animateAttack(who, moveType) {
    ...
    spawnChargeEffect(who, moveType);

    const chargeDelay = { electric: 260, fire: 310, water: 360, steel: 100, psychic: 460, neutral: 220 };
    await sleep(chargeDelay[moveType] || 250);
    ...
    // Launch projectile at lunge peak (~40% into the 0.65s animation)
    await sleep(260);
...
// Animated projectile — Canvas-based Pokémon-style attack effects
async function animateProjectile(fromWho, elementType) {
    ...
    // Create a canvas that exactly covers the fx layer
    const canvas = document.createElement('canvas');
    canvas.className = 'battle-fx-canvas';
    canvas.style.cssText = 'position:absolute;inset:0;pointer-events:none;z-index:22;';
    canvas.width  = fxLayer.offsetWidth  || 400;
    canvas.height = fxLayer.offsetHeight || 300;
    fxLayer.appendChild(canvas);
    const ctx = canvas.getContext('2d');

    const durations = { electric: 380, fire: 540, water: 620, steel: 180, psychic: 700, neutral: 450 };
    const totalMs = durations[elementType] || 450;
    ...
    await new Promise(resolve => {
        ...
        function frame(now) {
            ...
            drawTypeAttack(ctx, elementType, from, to, t);
...
// ─── Dispatch ─────────────────────────────────────────────────────────────────
function drawTypeAttack(ctx, type, from, to, t) {
    // Reset canvas state each frame
    ctx.shadowBlur  = 0;
    ctx.shadowColor = 'transparent';
    ctx.globalAlpha = 1;
    switch (type) {
        case 'electric': _drawElectric(ctx, from, to, t); break;
        case 'fire':     _drawFire(ctx, from, to, t);     break;
        case 'water':    _drawWater(ctx, from, to, t);    break;
        case 'steel':    _drawSteel(ctx, from, to, t);    break;
        case 'psychic':  _drawPsychic(ctx, from, to, t);  break;
        default:         _drawNeutral(ctx, from, to, t);  break;
    }
}

// ─── Electric: jagged lightning bolt extends across the field ─────────────────
function _drawElectric(ctx, from, to, t) {
    const progress = Math.min(t / 0.68, 1);
    const endX = from.x + (to.x - from.x) * _easeOut(progress);
    const endY = from.y + (to.y - from.y) * _easeOut(progress);
    const dx = to.x - from.x, dy = to.y - from.y;
    const len = Math.hypot(dx, dy) || 1;
    const perpX = -dy / len, perpY = dx / len;

    // Draw outer glow bolt, then white core
    [[6, 'rgba(255,200,0,0.45)'], [3, 'rgba(255,230,50,0.85)'], [1.5, 'rgba(255,255,200,1)']].forEach(([lw, color]) => {
        ctx.beginPath();
        ctx.strokeStyle = color;
        ctx.lineWidth = lw;
        ctx.shadowBlur  = lw === 1.5 ? 0 : 20;
        ...
        for (let i = 1; i <= segs; i++) {
            const s = i / segs;
            if (s > progress) {
                const partial = (progress - (i - 1) / segs) * segs;
                const nx = from.x + (endX - from.x) * ((i - 1 + partial) / segs);
                const ny = from.y + (endY - from.y) * ((i - 1 + partial) / segs);
                ctx.lineTo(nx, ny);
                break;
            }
            const mag = Math.sin(i * 31.7 + t * 25) * 22 + Math.cos(i * 7.3) * 10;
            ctx.lineTo(from.x + dx * s + perpX * mag, from.y + dy * s + perpY * mag);
        }
        ctx.stroke();
    });

    // Glowing sphere at bolt tip
    const tipR = 14 + Math.sin(t * 50) * 4;
    const g = ctx.createRadialGradient(endX, endY, 0, endX, endY, tipR);
    g.addColorStop(0,   'rgba(255,255,255,1)');
    g.addColorStop(0.4, 'rgba(255,230,0,0.8)');
    ...
    if (t > 0.65) {
        const a = ((t - 0.65) / 0.35) * 0.7;
        const ig = ctx.createRadialGradient(to.x, to.y, 0, to.x, to.y, 55);
        ig.addColorStop(0, `rgba(255,255,180,${a})`);
        ig.addColorStop(1, 'rgba(255,200,0,0)');
        ctx.fillStyle = ig;
        ctx.beginPath();
        ctx.arc(to.x, to.y, 55, 0, Math.PI * 2);
        ctx.fill();
    }
}

// ─── Fire: expanding fireball with trailing flame ─────────────────────────────
function _drawFire(ctx, from, to, t) {
    const arcY = -Math.sin(t * Math.PI) * 45;
    const cx = from.x + (to.x - from.x) * t;
    const cy = from.y + (to.y - from.y) * t + arcY;
    const size = Math.max(0, (20 + t * 50) * (t < 0.82 ? 1 : 1 - (t - 0.82) / 0.18));

    // Trailing embers (6 fading copies behind)
    for (let i = 1; i <= 6; i++) {
        const tr = Math.max(0, t - i * 0.065);
        const tArc = -Math.sin(tr * Math.PI) * 45;
        const tx = from.x + (to.x - from.x) * tr;
        const ty = from.y + (to.y - from.y) * tr + tArc;
        const ts = size * (1 - i * 0.14);
        const ta = (1 - i * 0.15) * 0.35;
        if (ts <= 0 || tr === 0) continue;
        const g = ctx.createRadialGradient(tx, ty, 0, tx, ty, ts);
        g.addColorStop(0,   `rgba(255,180,0,${ta})`);
        g.addColorStop(0.6, `rgba(255,60,0,${ta * 0.6})`);
    ...
    if (size > 0) {
        ...
        g.addColorStop(0.25,'rgba(255,180,0,0.95)');
        g.addColorStop(0.6, 'rgba(255,60,0,0.75)');
    ...
    // Impact bloom
    if (t > 0.78) {
        const a = ((t - 0.78) / 0.22) * 0.6;
        const ig = ctx.createRadialGradient(to.x, to.y, 0, to.x, to.y, 65);
        ig.addColorStop(0, `rgba(255,200,100,${a})`);
        ig.addColorStop(1, 'rgba(255,80,0,0)');
        ctx.fillStyle = ig;
        ctx.beginPath();
        ctx.arc(to.x, to.y, 65, 0, Math.PI * 2);
        ctx.fill();
    }
}

// ─── Water: bubble along high arc with ripple trail ───────────────────────────
function _drawWater(ctx, from, to, t) {
    const arcY = -Math.sin(t * Math.PI) * 80;
    const cx = from.x + (to.x - from.x) * t;
    const cy = from.y + (to.y - from.y) * t + arcY;
    const r  = 22 + Math.sin(t * 10) * 3;

    // Trail bubbles
    for (let i = 1; i <= 5; i++) {
        const tr = Math.max(0, t - i * 0.09);
        const tArc = -Math.sin(tr * Math.PI) * 80;
        const tx = from.x + (to.x - from.x) * tr;
        const ty = from.y + (to.y - from.y) * tr + tArc;
        const ta = (1 - i * 0.18) * 0.45;
        const ts = r * (1 - i * 0.15);
        if (ts <= 0 || tr === 0) continue;
        ctx.strokeStyle = `rgba(100,180,255,${ta})`;
        ctx.lineWidth = 2;
        ctx.shadowBlur  = 10;
    ...
    g.addColorStop(0.45, 'rgba(50,150,255,0.65)');
    g.addColorStop(0.8,  'rgba(0,90,220,0.85)');
    g.addColorStop(1,    'rgba(0,40,180,0.1)');
    ctx.fillStyle = g;
    ctx.beginPath();
    ...
    if (t > 0.8) {
        ...
        for (let l = 0; l < 3; l++) {
            const lr = (30 + l * 18) * ip;
            ctx.strokeStyle = `rgba(80,170,255,${(1 - ip) * 0.6})`;
            ctx.lineWidth = 2 - l * 0.4;
            ctx.beginPath();
            ctx.arc(to.x, to.y, lr, 0, Math.PI * 2);
            ctx.stroke();
        }
    }
}

// ─── Steel: multiple sharp shards fly in formation ────────────────────────────
function _drawSteel(ctx, from, to, t) {
    const SHARDS = 5;
    const dx = to.x - from.x, dy = to.y - from.y;
    const len = Math.hypot(dx, dy) || 1;
    const perpX = -dy / len, perpY = dx / len;
    ...
    for (let i = 0; i < SHARDS; i++) {
        const offset_t = Math.max(0, (t - i * 0.055) / 0.72);
        if (offset_t <= 0) continue;
        const p = Math.min(offset_t, 1);
        const spread = (i - (SHARDS - 1) / 2) * 16;
        const sx = from.x + dx * _easeIn(p) + perpX * spread;
        const sy = from.y + dy * _easeIn(p) + perpY * spread;
        ...
        ctx.shadowBlur  = 12;
        ctx.shadowColor = 'rgba(180,210,240,0.9)';
    ...
    if (t > 0.7) {
        const ip = (t - 0.7) / 0.3;
        ctx.strokeStyle = `rgba(200,220,255,${(1 - ip) * 0.8})`;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(to.x, to.y, 35 * ip + 5, 0, Math.PI * 2);
        ctx.stroke();
    }
}

// ─── Psychic: orb travels with expanding rings along its path ─────────────────
function _drawPsychic(ctx, from, to, t) {
    const cx = from.x + (to.x - from.x) * t;
    const cy = from.y + (to.y - from.y) * t;

    // Rings that spawn from the orb's current position and expand outward
    for (let r = 0; r < 4; r++) {
        const ringBirth = r * 0.15;
        if (t < ringBirth) continue;
        const age  = (t - ringBirth) * 1.4;
        const rad  = age * 50;
        const alph = Math.max(0, Math.sin(age * Math.PI * 0.85) * 0.65);

        const spawnX = from.x + (to.x - from.x) * ringBirth;
        const spawnY = from.y + (to.y - from.y) * ringBirth;

        ctx.strokeStyle = `rgba(200,80,255,${alph})`;
        ctx.lineWidth = 2.5;
        ctx.shadowBlur  = 14;
        ctx.shadowColor = 'rgba(160,0,255,0.5)';
        ctx.beginPath();
    ...
    // Main orb
    const orbR = 18 + Math.sin(t * 18) * 4;
    const orbA = t < 0.88 ? 1 : 1 - (t - 0.88) / 0.12;
    ctx.shadowBlur  = 28;
    ctx.shadowColor = 'rgba(180,0,255,0.8)';
    const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, orbR);
    g.addColorStop(0,   `rgba(255,255,255,${orbA})`);
    g.addColorStop(0.35,`rgba(220,80,255,${orbA * 0.9})`);
    g.addColorStop(0.7, `rgba(130,0,220,${orbA * 0.6})`);
...
// ─── Neutral: simple glowing projectile ───────────────────────────────────────
function _drawNeutral(ctx, from, to, t) {
    const cx = from.x + (to.x - from.x) * t;
    const cy = from.y + (to.y - from.y) * t;
    const r  = 16 * (t < 0.85 ? 1 : 1 - (t - 0.85) / 0.15);
    ctx.shadowBlur  = 18;
...
async function animateHit(who, moveType, damage, effectiveness, isCrit) {
    ...
    // Screen flash — always type-specific, plus super/crit override
    if (effectiveness > 1) screenFlash('super');
    else if (isCrit) screenFlash('crit');
    else screenFlash(moveType || 'neutral');
