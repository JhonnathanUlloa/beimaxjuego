...
async function createAdminUser() {
    try {
        ...
        db.run(
            'INSERT OR IGNORE INTO users (username, email, password, character_name, age, gender) VALUES (?, ?, ?, ?, ?, ?)',
            ['admin', 'admin@beimax.com', hashedPassword, 'AdminBot', 99, 'other'],
            function(err) {
                if (err) {
                    console.log('⚠️  Usuario admin ya existe o error:', err.message);
                } else if (this.lastID) {
                    const userId = this.lastID;
                    db.run('INSERT INTO user_profiles (user_id, coins, level, experience) VALUES (?, 10000, 50, 0)', [userId]);
                    db.run('INSERT INTO robot_customization (user_id) VALUES (?)', [userId]);
                    console.log('✅ Usuario admin creado: admin/admin (10000 monedas, nivel 50)');
                }
            }
        );
    } catch (error) {
